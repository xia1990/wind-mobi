...
#huangshaopeng@wind-mobi.com 20170807 start
function choose_X100()
{
    ...
    #    ALL_RELEASE_FILES="logo.bin "$PLATFORM"_Android_scatter.txt preloader_$BASE_PROJECT_NAME.bin AP_Database Modem_Database_lwg boot.img #secro.img userdata.img system.img lk.bin recovery.img cache.img trustzone.bin"#elif [ ! -f $OUT_PATH/Modem_Database_lwg ];then
...
case $build_param in
    ...
    WID016)
        ...
        ;;
    M100)
        BUILD_FILES="msm8953_64"
        ;;
    S100X)
        BUILD_FILES="msm8937_64"
        ;;
    S200X)
        BUILD_FILES="msm8937_64"
        ;;
...
WsRootDir=`pwd`
MY_NAME=`whoami`
OUT_PATH=$WsRootDir"/out/target/product"

OUT_PATH=$OUT_PATH/$BUILD_FILES

if [ ! -d $OUT_PATH ];then
    echo "ERROR: there is no out path:$OUT_PATH"
    exit 0
fi
#huangshaopeng@wind-mobi.com  20180108  add sdcard.img  begin
ALL_RELEASE_FILES="boot.img cache.img emmc_appsboot.mbn mdtp.img persist.img ramdisk.img recovery.img system.img userdata.img vendor.img"
#huangshaopeng@wind-mobi.com  20180108  add sdcard.img end
case $release_param in
    all)
        RELEASE_FILES=$ALL_RELEASE_FILES
        ;;
...
#release_verison
#huangshaopeng@wind-mobi.com 20170807 end
